...
def run():
    ...
    f = open('link-products.txt', 'a')
    for element in linkFile:
        print('link', element)
        browser.get(element)
        elms = browser.find_elements_by_xpath("//a[@href]")
        for elm in elms:
            if elm.get_attribute("href").startswith('https://www.lazada.vn/products/'):
                # links.append(elm.get_attribute("href"))
                f.writelines(elm.get_attribute("href") + '\n')
    f.close()
    browser.quit()
    # links.append(elm.get_attribute("href"))
    # print('link', links.append(elm.get_attribute("href"))
